...
function SkeletonCard({ featured = false }: { featured?: boolean }) {
  return (
    <div
      className={`bg-white/[0.03] border border-white/[0.07] rounded-2xl overflow-hidden animate-pulse ${featured ? "lg:flex" : ""}`}
    >
      <div
        className={`bg-white/[0.05] flex-shrink-0 ${featured ? "lg:w-[420px] h-56 lg:h-auto" : "h-48 sm:h-52"}`}
      />
...
function BlogCard({ post, featured = false }: { post: WordPressPost; featured?: boolean }) {
  ...
  return (
    <Link
      href={`/${post.slug}`}
      className={`group block bg-white/[0.025] border border-white/[0.07] rounded-2xl overflow-hidden hover:border-amber-500/35 hover:-translate-y-1 hover:shadow-2xl hover:shadow-amber-500/5 transition-all duration-300 ${featured ? "lg:flex" : ""}`}
    >
      {/* Image */}
      <div
        className={`relative overflow-hidden bg-white/[0.04] flex-shrink-0 ${featured ? "lg:w-[420px] h-56 lg:h-auto" : "h-48 sm:h-52"}`}
      >
        {img?.source_url ? (
          <img
            src={img.source_url}
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-amber-500/10 to-amber-900/5">
        ...
        {categories[0] && (
          <div className="absolute top-3 left-3">
            <span className="inline-flex items-center bg-amber-500 text-gray-950 text-[10px] font-black px-2.5 py-1 rounded-full">
              {categories[0].name}
            </span>
          </div>
        )}
      ...
      <div className={`p-5 flex flex-col ${featured ? "lg:p-7 justify-center" : ""}`}>
        ...
        {/* Title */}
        <h2
          className={`font-black text-white leading-snug tracking-tight mb-2 group-hover:text-amber-400 transition-colors ${featured ? "text-xl lg:text-2xl" : "text-base sm:text-lg"}`}
          dangerouslySetInnerHTML={{ __html: post.title.rendered }}
        />

        {/* Excerpt */}
        <p
          className={`text-white/45 leading-relaxed ${featured ? "text-sm mb-5 line-clamp-3" : "text-xs mb-4 line-clamp-2"}`}
        >
          {excerpt}…
        </p>

        {/* Read more */}
        <div className="mt-auto flex items-center gap-1.5 text-amber-400 font-bold text-xs group-hover:gap-2.5 transition-all">
          Read Article
...
function SectionLabel({ label, count }: { label: string; count?: string }) {
  return (
    <div className="flex items-center gap-3 mb-6">
      ...
      <span className="text-amber-500 text-xs font-bold uppercase tracking-[0.16em]">{label}</span>
      <div className="h-px flex-1 bg-white/[0.06]" />
      {count && <span className="text-xs text-white/30 font-medium">{count}</span>}
    </div>
  );
}
...
export default function BlogsPage() {
  ...
  return (
    <div className="bg-[#0A0A0A] text-white min-h-screen">
      ...
      {/* ══ HERO ══════════════════════════════════════════════════════════ */}
      <section className="relative pt-28 pb-16 overflow-hidden border-b border-white/[0.06]">
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 relative z-10">
          ...
          <nav className="flex items-center gap-1.5 text-xs text-white/30 mb-8 flex-wrap">
            <Link href="/" className="hover:text-amber-400 transition-colors font-medium">Home</Link>
            <ChevronRight className="w-3 h-3" />
            <span className="text-white/60 font-medium">Blog</span>
          </nav>
          ...
          <div className="max-w-2xl">
            ...
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black tracking-tight leading-[1.05] mb-5">
              Learn From India&apos;s{" "}
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 py-14 lg:py-20">
        ...
        {!loading && error && (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <h2 className="text-xl font-black text-white mb-2">Failed to load posts</h2>
            <p className="text-white/45 text-sm mb-6 max-w-xs">
              Something went wrong. Please try again.
            </p>
            <button
              onClick={() => window.location.reload()}
              className="inline-flex items-center gap-2 bg-amber-500 hover:bg-amber-400 text-gray-950 font-black px-6 py-3 rounded-xl text-sm transition-all shadow-lg shadow-amber-500/20"
            >
              Try Again
            </button>
          </div>
        ...
        {!loading && !error && posts.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <div className="w-16 h-16 bg-amber-500/10 border border-amber-500/20 rounded-2xl flex items-center justify-center mb-5">
        ...
        {!loading && !error && posts.length > 0 && (
          <>
            ...
            {featuredPost && (
              <div className="mb-14">
                <SectionLabel label="Featured Post" />
              ...
              <div className="mb-12">
                ...
                <div className="flex flex-wrap gap-2">
                  {categories.map((cat) => (
                    <div
                      key={cat.id}
                      className="inline-flex items-center gap-1.5 bg-white/[0.04] border border-white/[0.08] text-white/55 text-xs font-semibold px-3.5 py-2 rounded-full hover:bg-amber-500/10 hover:border-amber-500/30 hover:text-amber-400 transition-all cursor-default"
                    >
                      {cat.name}
            ...
            {recentPosts.length > 0 && (
              <div className="mb-14">
                <SectionLabel label="Recent Articles" />
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            {remainingPosts.length > 0 && (
              <div>
                <SectionLabel label="All Articles" count={`${remainingPosts.length} posts`} />
      ...
      {/* ══ CTA STRIP ═════════════════════════════════════════════════════ */}
      <section className="border-t border-white/[0.06] bg-[#0D0D0D] py-14 relative overflow-hidden">
        <div
        ...
        <div className="max-w-4xl mx-auto px-5 sm:px-6 lg:px-8 relative z-10 flex flex-col sm:flex-row items-center justify-between gap-6 text-center sm:text-left">
          <div>
          ...
          <Link
            href="/contact"
            className="flex-shrink-0 inline-flex items-center gap-2 bg-amber-500 hover:bg-amber-400 text-gray-950 font-black px-7 py-3.5 rounded-2xl hover:-translate-y-0.5 transition-all shadow-xl shadow-amber-500/20 text-sm group"
          >
            Get Free Audit
